...
const { CoinManager, DatabaseClient } = require("./app");
	...
	if(this.playfield[val]>0){
		...
		if(done){
			...
			if(this.playfield[6]>24){
                ...
                DatabaseClient.setPoints(this.players[0], users[this.players[0]].bank);
			}
                ...
                DatabaseClient.setPoints(this.players[1], users[this.players[1]].bank);
			}
...
module.game.prototype.quit=function(player,users,games,openGames){
	if(this.status==='open'){
        ...
        DatabaseClient.setPoints(player, users[player].bank);
		delete games[this.server][this.players[0]];
	...
	for(let i=0;i<2;i++){
		if(this.players[i]===player){
            ...
            DatabaseClient.setPoints(this.players[i], users[this.players[i]].bank);
		}
